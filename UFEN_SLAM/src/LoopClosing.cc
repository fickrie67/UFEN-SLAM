                                             ...
                                             int &nNumCoincidences, std::vector<MapPoint*> &vpMPs, std::vector<MapPoint*> &vpMatchedMPs)
{
    ...
    int nSim3Inliers = 20;
    int nProjMatches = 50;
    int nProjOptMatches = 80;  // tune these values if the loop-closure detection is hard
    ...
    for(KeyFrame* pKFi : vpBowCand)
    {
        ...
        if(!bAbortByNearKF && numBoWMatches >= nBoWMatches) // TODO pick a good threshold
        {
            ...
            solver.SetRansacParameters(0.99, nBoWInliers, 300); // at least 15 inliers 0.99
            ...
            if(bConverge)
            {
                ...
                if(numProjMatches >= nProjMatches)
                {
                    ...
                    int numOptMatches = Optimizer::OptimizeSim3(mpCurrentKF, pKFi, vpMatchedMP, gScm, 20, mbFixScale, mHessian7x7, true); 
                    ...
                    if(numOptMatches >= nSim3Inliers)
                    {
                        ...
                        vector<MapPoint*> vpMatchedMP;
                        vpMatchedMP.resize(mpCurrentKF->GetMapPointMatches().size(), static_cast<MapPoint*>(NULL));
                        int numProjOptMatches = matcher.SearchByProjection(mpCurrentKF, mScw, vpMapPoints, vpMatchedMP, 5, 2.5); 
                                         ...
                                         vector<MapPoint*> &vpMatchedMapPoints)
{
    ...
    int num_matches = matcher.SearchByProjection(pCurrentKF, mScw, vpMapPoints, vpMatchedMapPoints, 3, 2);
