...
void Tracking::MonocularInitialization()
{
    ...
    else
    {
        ...
        if(mLastFrame.mTimeStamp-mInitialFrame.mTimeStamp>0.8) 
        {
            delete mpInitializer;
            mpInitializer = static_cast<Initializer*>(NULL);
            fill(mvIniMatches.begin(),mvIniMatches.end(),-1);
...
bool Tracking::TrackWithMotionModel()
{
    ...
    //int nmatches = matcher.SearchByNN(mCurrentFrame,mLastFrame,th,mSensor==System::MONOCULAR || mSensor==System::IMU_MONOCULAR);
...
bool Tracking::TrackLocalMap()
{
...
#ifdef REGISTER_TIMESSearchLocalPoints
    ...
    {
        if(mnMatchesInliers<5) //30 lower threshold
            return false;
        else
            return true;
    }
...
void Tracking::SearchLocalPoints()
{
    ...
    if(nToMatch>0)
    {
        ...
        //int matches = matcher.SearchByNN(mCurrentFrame,mvpLocalMapPoints, mK); //try BNN matching
        int matches = matcher.SearchByNN(mCurrentFrame, mvpLocalMapPoints, th, mpLocalMapper->mbFarPoints, mpLocalMapper->mThFarPoints); //try BNN matching
    }
}
