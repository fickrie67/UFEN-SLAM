...
int ORBmatcher::SearchByNN(Frame &F, const vector<MapPoint*> &vpMapPoints, const float th, const bool bFarPoints, const float thFarPoints)
{
    ...
    for(size_t iMP = 0; iMP < vpMapPoints.size(); iMP++)
    {
        ...
        projectedPoints.push_back(cv::Point2f(pMP->mTrackProjX, pMP->mTrackProjY));

        mpIndices.push_back(iMP);
    }

    if (MPdescriptorAll.empty())
        return 0;

    cv::Mat MPdescriptors((int)MPdescriptorAll.size(), 32, CV_8U);
    for (int i = 0; i < MPdescriptorAll.size(); ++i)
        MPdescriptorAll[i].copyTo(MPdescriptors.row(i));

    std::vector<cv::DMatch> matches;
    cv::BFMatcher matcher(cv::NORM_HAMMING, true);
    matcher.match(MPdescriptors, F.mDescriptors, matches);
    ...
    {
        const int idxMP = select_indice[m.queryIdx];
        const int idxF = m.trainIdx;
        if(idxF < 0 || idxF >= F.mvKeysUn.size()) continue;

        points1.push_back(projectedPoints[m.queryIdx]);             
        points2.push_back(F.mvKeysUn[idxF].pt);                     
        validIndices.push_back(m.trainIdx);                         
    }
    ...
    cv::Mat F_matrix = cv::findFundamentalMat(points1, points2, cv::FM_RANSAC, 10.0, 0.99, inliersF);
    cv::Mat H_matrix = cv::findHomography(points1, points2, cv::RANSAC, 10.0, inliersH);

    std::vector<int> combinedInliers(inliersF.size(), 0);
    for (size_t i = 0; i < inliersF.size(); ++i) {
        //combinedInliers[i] = inliersF[i]|| inliersH[i];
        combinedInliers[i] = inliersH[i];
    }


    int nmatches = 0;
    for (size_t i = 0; i < combinedInliers.size(); ++i)
    {
        if (!combinedInliers[i]) continue;

        const int idxMP = select_indice[matches[i].queryIdx];
        const int idxF  = matches[i].trainIdx;

        if(idxF < 0 || idxF >= F.mvpMapPoints.size()) continue;

        if(F.mvpMapPoints[idxF])
            if(F.mvpMapPoints[idxF]->Observations() > 0)
                continue;

        F.mvpMapPoints[idxF] = vpMapPoints[idxMP];
        nmatches++;
    }
...
int ORBmatcher::SearchByNN(Frame &F, const vector<MapPoint*> &vpMapPoints, const cv::Mat &K)
{
    ...
    {
        MapPoint* pMP = vpMapPoints[select_indice[matches[i].queryIdx]];
        if(pMP)
        {
            points1.push_back(cv::Point2f(pMP->GetWorldPos().at<float>(0), pMP->GetWorldPos().at<float>(1)));
            points2.push_back(F.mvKeysUn[matches[i].trainIdx].pt);
        }
    }
    ...
    cv::Mat H_matrix = cv::findHomography(points1, points2, cv::RANSAC, 20.0, inliersH);

    std::vector<int> combinedInliers(inliersF.size(), 0);
    for (size_t i = 0; i < inliersF.size(); ++i) {
        // Union of inliers
        combinedInliers[i] = true; //inliersF[i]|| inliersH[i];
        //combinedInliers[i] = inliersH[i];
    }
    int nmatches = 0;
    for (int i = 0; i < static_cast<int>(matches.size()); ++i)
    {
        if (combinedInliers[i])
        {
    ...
    int num_inliers = std::count(combinedInliers.begin(), combinedInliers.end(), 1);
...
int ORBmatcher::SearchByNN(KeyFrame *pKF, Frame &F, std::vector<MapPoint*> &vpMapPointMatches)
{
    ...
    // Extract matched keypoints
    std::vector<cv::Point2f> points1, points2;
    std::vector<int> select_indice(matches.size());
    for (int i = 0; i < static_cast<int>(matches.size()); ++i)
    {
        ...
        {
            points1.push_back(cv::Point2f(pMP->GetWorldPos().at<float>(0), pMP->GetWorldPos().at<float>(1)));
            points2.push_back(F.mvKeysUn[bestIdxF].pt);
            select_indice[i] = realIdxKF;
        }
    }
    int nmatches =0;
    for (int i = 0; i < static_cast<int>(matches.size()); ++i) {
...
int ORBmatcher::Fuse(KeyFrame *pKF, const vector<MapPoint *> &vpMapPoints, float th, const bool bRight)
{
    cv::Mat Rcw,tcw, Ow;
    GeometricCamera* pCamera;
    th = 3;
    // std::cout << th << std::endl; //try
