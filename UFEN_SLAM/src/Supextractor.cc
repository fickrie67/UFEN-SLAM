...
Supextractor::Supextractor(int _nfeatures, float _scaleFactor, int _nlevels,
    ...
    iniThFAST(_iniThFAST), minThFAST(_minThFAST)
{
    ...
    const char *net_fn = getenv("Sup_PATH");
    net_fn = (net_fn == nullptr) ? "gcn2.pt" : net_fn;
    module = torch::jit::load(net_fn, device);

}

void Supextractor::operator()( InputArray _image, InputArray _mask, vector<KeyPoint>& _keypoints, OutputArray _descriptors)
{ 
