    ...
    pCamera1(pKF1->mpCamera), pCamera2(pKF2->mpCamera)
{
    ...
    for(int i1=0; i1<mN1; i1++)
    {
        if(vpMatched12[i1])
        {
            ...
            mvnMaxError1.push_back(9.210*sigmaSquare1 * 2); //try
            mvnMaxError2.push_back(9.210*sigmaSquare2 * 2);
...
void Sim3Solver::CheckInliers_loopc()
{
    vector<cv::Mat> vP1im2, vP2im1;
    Project(mvX3Dc2,vP2im1,mT12i,pCamera1);
    Project(mvX3Dc1,vP1im2,mT21i,pCamera2);
    ...
    for(size_t i=0; i<mvP1im1.size(); i++)
    {
        ...
        if(err1<mvnMaxError1[i]*5.0 && err2<mvnMaxError2[i]*5.0)
        {
            mvbInliersi[i]=true;
            mnInliersi++;
        }
